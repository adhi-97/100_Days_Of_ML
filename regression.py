...
insurance_dataframe = insurance_dataframe.reindex(
    np.random.permutation(insurance_dataframe.index))  # randomizing the dataset
...
def my_input_fn(features, targets, batch_size=1, shuffle=True, num_epochs=None):
    ...
    ds = ds.batch(batch_size).repeat(
        num_epochs)  # constructing a dataset object from our data, and then break our data into batches of batch_size, to be repeated for the specified number of epochs.
    if shuffle:
...
def train_model(learning_rate, steps, batch_size, input_feature="X"):
    ...
    prediction_input_fn = lambda: my_input_fn(my_feature_data, targets, num_epochs=1,
                                              shuffle=False)  # input function for prediction
...
train_model(learning_rate=0.0002, steps=50000, batch_size=1)  # training the model
